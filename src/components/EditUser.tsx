...
interface CreateDriverProps {
    ...
    phoneIn: string,
    passwordIn: string,
    activationCodeIn: string,
    activeIn: boolean,
    roleIn: string,
    uuidIn: string
}

export function EditUser({onCreate, loginIn, emailIn, phoneIn, passwordIn, activationCodeIn, activeIn, roleIn, uuidIn}: CreateDriverProps){
    const [roles, setRoles] = useState<IRole[]>([])
    const [err, setError] = useState('')

    const [valueLogin, setValueLogin] = useState(loginIn)
    const [valueEmail, setValueEmail] = useState(emailIn)
    const [valuePhone, setValuePhone] = useState(phoneIn)
    const [valuePassword, setValuePassword] = useState(passwordIn)
    const [valueActivationCode, setValueActivationCode] = useState(activationCodeIn)
    const [valueActive, setValueActive] = useState(activeIn)
    const [valueRole, setValueRole] = useState(roleIn)
    ...
    const submitHandler = async (event: React.FormEvent) => {
        ...
        try{
            const response = await axios.post<IUser>('http://localhost:8080/saveuser', userData)
